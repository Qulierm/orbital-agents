...
/** Targets for the blank-peer chrome activation (session shell scope). */
export interface PeerActivationTargets {
  readonly currentSession: () => string | undefined
  readonly subscribeCurrent: (listener: () => void) => () => void
  readonly peerFace: (sessionId: string, key: string) => PeerProjectionFace | undefined
  /** Official, event-free activation of the session's Chat target. */
  readonly activateConversation: (sessionId: string) => void
}

/**
 * Activate the registered Chat target for the CHALLENGER side of a pair, ONCE
 * per session. A freshly provisioned peer has no turns, so without this the app
 * renders the empty hero (no header, no View ring) and the reciprocal Endeavour
 * tab would be unreachable. Activation is a client-side view-state change: no
 * durable event is appended, no prompt is sent and no model request starts.
 * Idempotent across re-mounts, session switches and projection updates.
 */
export function reconcilePeerActivation(targets: PeerActivationTargets): () => void {
  ...
  const refresh = (): void => {
    ...
    const role = peerTabTarget(sessionId, peer)?.role
    if (role !== 'challenger') {
      // The projection may arrive right after the session shell mounts; retry a
      // bounded number of times instead of waiting for another session switch.
